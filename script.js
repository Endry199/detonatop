...
// Elemento para mostrar el total del clan (DEBES AÑADIR ESTO EN TU HTML)
// <div id="clan-total-points-container">Puntos Totales del Clan: <span id="clan-total-points">0</span></div>
const clanTotalPointsSpan = document.getElementById('clan-total-points');
...
// *** Lógica para guardar y restaurar el estado de las bandejas (Punto 3) ***
const EXPANDED_GROUPS_KEY = 'expandedGroups';

function saveExpandedState() {
    const expandedGroups = [];
    gruposTableBody.querySelectorAll('.expanded-content:not(.hidden)').forEach(row => {
        expandedGroups.push(row.dataset.groupId);
    });
    localStorage.setItem(EXPANDED_GROUPS_KEY, JSON.stringify(expandedGroups));
}

async function restoreExpandedState() {
    const expandedGroups = JSON.parse(localStorage.getItem(EXPANDED_GROUPS_KEY) || '[]');
    for (const groupId of expandedGroups) {
        const groupRow = gruposTableBody.querySelector(`.group-row[data-group-id="${groupId}"]`);
        if (groupRow) {
            // Usa un evento de clic para disparar la lógica de renderizado
            groupRow.click();
        }
    }
}
...
    // *** Punto 2: Cargar y mostrar los puntos totales del clan ***
    const { data: allGroups, error: allGroupsError } = await supabase
        .from('grupos')
        .select('puntos_totales');

    if (!allGroupsError && clanTotalPointsSpan) {
        const totalPuntosClan = allGroups.reduce((sum, grupo) => sum + (grupo.puntos_totales || 0), 0);
        clanTotalPointsSpan.textContent = totalPuntosClan;
    }
    
...
    });
    // Restaurar el estado de las bandejas al renderizar los grupos.
    restoreExpandedState();
}

// Función auxiliar para asegurar la estructura de escuadras y miembros
async function ensureGroupStructure(groupId) {
    // ... (el resto de esta función se mantiene igual, no es necesario modificarla) ...
...
    // Se reordena el llamado a `ensureGroupStructure` para poder ordenar antes de renderizar
...
    if (escuadrasError) {
        console.error('Error al obtener escuadras:', escuadrasError);
        containerElement.innerHTML = `<div class="escuadras-container"><p>Error al cargar las escuadras.</p></div>`;
        return;
    }
    
    // *** Punto 1: Lógica para ordenar las escuadras ***
    const totalPuntosEscuadras = escuadras.reduce((sum, escuadra) => sum + (escuadra.puntos_totales || 0), 0);
    let escuadrasOrdenadas = escuadras;

    if (totalPuntosEscuadras > 0) {
        // Si hay puntos, ordenar por puntos de mayor a menor
        escuadrasOrdenadas.sort((a, b) => b.puntos_totales - a.puntos_totales);
    } else {
        // Si todos los puntos son cero, ordenar por ID (Escuadra 1, 2, 3...)
        escuadrasOrdenadas.sort((a, b) => a.id - b.id);
    }

    // Ahorra un re-render innecesario de ensureGroupStructure, ya que solo necesitamos sus datos
    const finalEscuadras = await ensureGroupStructure(groupId);
    
...
    if (escuadrasOrdenadas.length === 0) {
        containerElement.innerHTML = `<div class="escuadras-container"><p>No hay escuadras en este grupo.</p></div>`;
        return;
    }

    let escuadrasHtml = '';
    for (const escuadra of escuadrasOrdenadas) { // Usa las escuadras ordenadas aquí
        const miembros = escuadra.miembros_del_clan;
...
            // Limpiar el estado de las bandejas al cerrar sesión
...
                // Guardar el estado de las bandejas cada vez que se abre o cierra una
                saveExpandedState(); 
