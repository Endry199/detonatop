...
// Objeto para guardar el estado de las bandejas expandidas
const expandedRows = {};
...
// Función para actualizar los puntos de un miembro y refrescar la vista
async function updateMemberPointsAndRefresh(miembroId) {
    ...
    if (miembroError) {
        console.error('Error al obtener el miembro:', miembroError);
        return;
    }

    const { data: escuadra, error: escuadraError } = await supabase
        .from('escuadras')
        .select('id_grupo')
        .eq('id', miembro.id_escuadra)
        .single();
    
    if (escuadraError) {
        console.error('Error al obtener la escuadra:', escuadraError);
        return;
    }

    const groupId = escuadra.id_grupo;

    // Actualizar solo el grupo y las escuadras relacionadas
    const expandedContent = gruposTableBody.querySelector(`.expanded-content[data-group-id="${groupId}"]`);
    if (expandedContent) {
        expandedContent.querySelector('td').innerHTML = `<div class="loading">Cargando...</div>`;
        const puedeEditar = (currentRole === 'admin') || (['lider', 'decano'].includes(currentRole) && currentGroupId == groupId);
        await renderEscuadrasEnGrupo(groupId, expandedContent.querySelector('td'), puedeEditar);
    }
    
    // Y actualizar el puntaje general del grupo en la tabla principal
    const groupRow = gruposTableBody.querySelector(`.group-row[data-group-id="${groupId}"]`);
    if (groupRow) {
        const { data: updatedGroup, error: groupError } = await supabase
            ...
            .single();
        if (!groupError) {
            groupRow.querySelector('td:last-child').textContent = updatedGroup.puntos_totales || 0;
        }
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Revisa si hay filas expandidas que deben mantenerse abiertas
    for (const groupId in expandedRows) {
...
                
                // Guardar el estado de la bandeja
                expandedRows[groupId] = !expandedRow.classList.contains('hidden');
                
...
                console.error(error);
            } else {
                // Llama a la nueva función para refrescar la vista
                await updateMemberPointsAndRefresh(miembroId);
            }
        }
    });
});

// Nuevo evento para detectar cuando la página recupera el foco
window.addEventListener('focus', async () => {
    // Solo refresca los grupos si no hay un usuario logueado o si no se ha expandido una fila
    if (!supabase.auth.session() || Object.keys(expandedRows).length === 0) {
        await renderGrupos();
    } else {
        // Si hay un usuario logueado y hay filas expandidas, solo actualiza esas filas
        for (const groupId in expandedRows) {
            if (expandedRows[groupId]) {
                const expandedRow = gruposTableBody.querySelector(`.expanded-content[data-group-id="${groupId}"]`);
                if (expandedRow) {
